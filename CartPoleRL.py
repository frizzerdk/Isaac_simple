...
def __main__():
    ...
    while not gym.query_viewer_has_closed(viewer):
        ...
        if counter > 10:
            counter = 0
            show_value_function(device,value_function=lambda states : compute_value_function_agent(states,agent,device),name='Agent Value Function',window_position=(1050,1000))
            show_value_function(device,value_function=lambda states : compute_policy_function_agent(states,agent,device),name='Agent Policy Function',window_position=(550,1000))
...
def compute_q_function_agent(states,agent,device):
    states = states.clone().detach()*torch.tensor([[300,4,4,20]],device=device)
...
def compute_value_function_agent(states,agent,device):
    states = states.clone().detach()*torch.tensor([[4,4,4,20]],device=device)
...
def show_value_function(device, value_function=None, limits=None, index_map=None, name='Value Function',window_position = None):
    ...
    value_function = compute_value_function if value_function is None else value_function
...
def calculate_rewards(s1,a1,s2,num_env,reset_buf):
    ...
    reset_rewards = reset_buf.squeeze()*-100
